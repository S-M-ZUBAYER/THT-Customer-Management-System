...
const UserContext_CN = ({ children }) => {
    const [user, setUser] = useState(null);
    const [chattingUser, setChattingUser] = useState(localStorage.getItem("chattingUser"));
    ...
    const fetchUserByUserId = async () => {
        console.log(chattingUser?.userId, "check the list")
        try {
            // const response = await axios.get(`https://jiapuv.com:3091/CustomerService-ChatCN/api/dev/chatlist/customer_service/${chattingUser?.userId}`);
            const response = await axios.get(`https://jiapuv.com:3091/CustomerService-ChatCN/api/dev/chatlist/customer_service/${chattingUser?.userId}`);
    ...
    console.log(chattingUser, "chatting user from userContext_CN");
