...
const handleDelete = async (userId) => {
  try {
    await axios.delete(`http://localhost:2000/tht/QandAnswers/delete/${userId}`);
...
const updateUser = async (userId, editingUser) => {
  try {
    const response = await axios.put(`http://localhost:2000/tht/QandAnswers/update/${userId}`, editingUser);
...
// };
 ...
 console.log(user?.email,question,answer,date,time)
      ...
      fetch('http://localhost:2000/tht/QandAnswers/add', {
          method: 'POST',
          headers: {
  ...
  const handleEdit = async(id, editedQuestion, editedAnswer, editedDate, editedTime) => {
    ...
    try {
      const response = await axios.put(`http://localhost:2000/tht/QandAnswers/update/${id}`, {editedQuestion, editedAnswer, editedDate, editedTime});
      toast.success("user information updated successfully");
      // Optionally, you can show a success message to the user using a toast or other UI notification.
    } catch (error) {
      toast.error('Error updating user:', error);
